...
<head>
    ...
    <title>订单列表页-综合网购首选-正品低价、品质保障、配送及时、轻松购物!
    </title>
    <!-- 网站说明 -->
    ...
    <script type="text/javascript">
        function showOrder(oid) {
            location.href = "orders?method=detail&oid=" + oid;
        }
    </script>
</head>
...
<div class="sk_container w">
    ...
    <div class="sk_bd">
        <c:if test="${empty ordersList}">
            <h3 style="font-size: 20px; color: #000;font-weight: 400;">您还没有订单，先去愉快地购物吧！</h3>
        </c:if>
                ...
                <c:forEach items="${ordersList}" var="o" varStatus="status">
                    <tr>
                        ...
                        <c:if test="${o.ostate == 0}">
                            <td style="color: black">未付款</td>
                        </c:if>
                        <c:if test="${o.ostate == 1}">
                            <td style="color: blue">已付款，未发货</td>
                        </c:if>
                        <c:if test="${o.ostate == 2}">
                            <td style="color: darkorange">已发货，待收货</td>
                        </c:if>
                        <c:if test="${o.ostate == 3}">
                            <td style="color: green">已收获，待评价</td>
                        </c:if>
                        <c:if test="${o.ostate == 4}">
                            <td style="color: hotpink">已评价</td>
                        </c:if>
                        <c:if test="${o.ostate == 5}">
                            <td style="color: red">退款</td>
                        </c:if>
                        <td>${o.otime}</td>
                        <td>${o.address.adetail}</td>
                        <td>
                            <a class="detail" href="orders?method=detail&oid=${o.oid}">订单详情</a>
                        </td>
                    </tr>
                </c:forEach>
            </table>
