...
<div class="sk_container w">
    ...
    <c:if test="${empty listCart}">
        <div class="sk_bd">
            <h2 style="color: #000000; font-weight: 400;">您的购物车中没有任何商品！</h2>
        </div>
    </c:if>
            ...
            <table>
                ...
                <c:forEach items="${listCart}" var="cart" varStatus="status">
                    <tr>
                        ...
                        <td>${cart.product.pprice}</td>
                        <td>
                        <span class="input-group-btn">
                            <button onclick="sub(${cart.cid}, ${cart.cnum}, ${cart.product.pprice})">-</button>
                        </span>
                            <input type="text" value="${cart.cnum}" class="num">
                            <span class="input-group-btn">
                            <button onclick="add(${cart.cid}, ${cart.cnum}, ${cart.product.pprice})">+</button>
                        </span>
                        </td>
                        <td>￥${cart.ccount}</td>
                        <td>
                            <button class="delete" onclick="delCart(${cart.cid})">删除</button>
                        </td>
                    </tr>
                    <c:set var="sum" value="${sum + cart.ccount}"></c:set>
                </c:forEach>
            </table>
        </div>
        <div class="sk_fd">
            <div class="sum">商品金额总计：<i class="style_red">￥${sum}</i></div>
            <div class="btn">
                <button class="btn1" onclick="clearCart(${loginUser.uid})">清空购物车</button>
                <a href="orders?method=preview&uid=${loginUser.uid}" class="btn2">查看订单预览</a>
            </div>
        </div>
    </c:if>
</div>
