...
<div class="sk_container w">
    ...
    <div class="sk_bd">
        ...
        <div class="addr">
            <c:if test="${empty addressList}">
                <h3 style="font-size: 18px;">您还没有收货地址，请先在个人中心进行添加！</h3>
            </c:if>
            <c:if test="${not empty addressList}">
                ...
                <c:forEach items="${addressList}" var="a" varStatus="status">
                    <c:if test="${a.astate == 1}">
                        <option selected="selected" value="${a.aid}">${a.aname}&nbsp;&nbsp;${a.aphone}&nbsp;&nbsp;${a.adetail}</option>
                    </c:if>
    ...
    <div class="sk_fd">
        ...
        <div class="btn">
            <a href="cart?method=show" class="btn1">返回购物车</a>
            <c:if test="${empty addressList}">
                <a href="address?method=show" class="btn2">添加地址</a>
            </c:if>
            <c:if test="${not empty addressList}">
                <button class="btn3" onclick="createOrder(${loginUser.uid}, ${sum})">提交订单</button>
            </c:if>
        </div>
    </div>
</div>
